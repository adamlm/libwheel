...
struct R2Vector {
    ...
    static auto interpolate(const R2Vector &source, const R2Vector &target, std::size_t num_samples)
        -> std::vector<wheel::R2Vector> {
        std::vector<wheel::R2Vector> configs;

        const auto step_x{(target.x - source.x) / static_cast<decltype(source.x)>(num_samples)};
        const auto step_y{(target.y - source.y) / static_cast<decltype(source.y)>(num_samples)};

        for (auto i{1U}; i <= num_samples; ++i) {
